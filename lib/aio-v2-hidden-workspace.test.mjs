...
test('AIO v2 planning route is exposed and uses an async Next 15 route parameter', async () => {
...
test('AIO v2 workspace checks entitlement before requesting job-scoped data', async () => {
  ...
  assert.match(source, /Contact an administrator for help/)
})
...
test('AIO v2 history and launcher are visible while keeping Create and Refresh explicit', async () => {
...
test('AIO v2 history and launcher check entitlement before owner data or capabilities', async () => {
  ...
  assert.match(jobs, /Contact an administrator for help/)
  assert.match(launcher, /Contact an administrator for help/)
})
