...
test('shared tokens include hover, switch track, and themed scrollbar support', () => {
  ...
  assert.match(globals, /--radius-control:\s*6px/)
  assert.match(globals, /--radius-icon:\s*4px/)
  assert.match(globals, /\.dropdown-menu,[\s\S]*?\.cp-menu\s*\{[\s\S]*?border-radius:\s*var\(--radius-control\)/)
  assert.match(globals, /\.cs-trigger\s*\{[\s\S]*?border-radius:\s*var\(--radius-control\)/)
  assert.match(globals, /\.cs-panel\s*\{[\s\S]*?border-radius:\s*var\(--radius-control\)/)
