...
test('Brand Pulse is available under insights navigation', async () => {
  ...
  const insights = sidebar.match(/label:\s*'Insights',\s*items:\s*\[([\s\S]*?)\]\s*,?\s*}/)?.[1] ?? ''
  assert.match(sidebar, /const NAV_GROUPS:/)
  assert.match(insights, /href:\s*'\/brand-mentions'[\s\S]*label:\s*'Brand Pulse'/)
  assert.match(sidebar, /label:\s*'Create'[\s\S]*label:\s*'Insights'[\s\S]*label:\s*'Operations'/)
...
test('Brand Pulse detail can expand duplicate mention groups locally', async () => {
  const detail = await readFile(new URL('../app/(app)/brand-mentions/[id]/page.tsx', import.meta.url), 'utf8')
  const css = await readFile(new URL('../app/globals.css', import.meta.url), 'utf8')

  assert.match(detail, /include_duplicates/)
  assert.match(detail, /set\('include_duplicates', 'true'\)/)
  assert.match(detail, /expandedDuplicateKeys/)
  assert.match(detail, /toggleDuplicateGroup/)
  assert.match(detail, /collapseDuplicateGroups/)
  assert.match(detail, /brand-pulse-duplicate-toggle/)
  assert.match(detail, /Show similar/)
  assert.match(detail, /Hide similar/)
  assert.match(css, /\.brand-pulse-duplicate-toggle/)
})
...
test('Brand Pulse detail shows existing mention timing and coverage signals', async () => {
  ...
  assert.match(detail, /DFS insights/)
  assert.match(detail, /Coverage details/)
  assert.match(detail, /DFS source mix/)
  assert.match(detail, /DFS sentiment/)
  assert.doesNotMatch(detail, /buildRecentMentionDays/)
  assert.doesNotMatch(detail, /Source mix<\/p>/)
  assert.doesNotMatch(detail, /Category mix<\/p>/)
...
test('Brand Pulse detail page exposes per-crawl DFS row controls with guardrails', async () => {
  ...
  assert.match(api, /pull_mode\?: 'newest' \| 'best_quality' \| 'negative_watch' \| 'one_per_domain'/)
  assert.match(api, /filters\?: \{/)
  assert.match(detail, /const DFS_ROW_PRESETS = \[50, 100, 250, 500, 1000\]/)
  assert.match(detail, /const DFS_PULL_MODE_OPTIONS/)
  assert.match(detail, /const DFS_PROVIDER_SENTIMENT_OPTIONS/)
  assert.match(detail, /Pull/)
  assert.match(detail, /Newest mentions/)
  assert.match(detail, /Best quality/)
  assert.match(detail, /Negative watch/)
  assert.match(detail, /One per domain/)
  assert.match(detail, /buildDfsCrawlFilters/)
  assert.match(detail, /showDfsFilters/)
  assert.match(detail, /Provider sentiment/)
  assert.match(detail, /Include domain/)
  assert.match(detail, /Exclude domain/)
  assert.match(detail, /From date/)
  ...
  assert.match(detail, /const payload: BrandMentionCrawlPayload/)
  assert.match(detail, /if \(selectedDfsFilterCount > 0\) payload\.filters = selectedDfsFilters/)
  assert.match(detail, /brandMentionsApi\.crawlAlert\(token, alertId, payload\)/)
})
...
test('Brand Pulse detail exposes latest crawl summary and quick review filters', async () => {
  const detail = await readFile(new URL('../app/(app)/brand-mentions/[id]/page.tsx', import.meta.url), 'utf8')

  assert.match(detail, /latestCrawlRun/)
  assert.match(detail, /latestCrawlSummaryItems/)
  assert.match(detail, /brand-pulse-latest-crawl-strip/)
  assert.match(detail, /Rows checked/)
  assert.match(detail, /Seen again/)
  assert.match(detail, /function applyQuickReviewFilter/)
  assert.match(detail, /New only/)
  assert.match(detail, /Updated only/)
  assert.match(detail, /Needs review/)
  assert.match(detail, /setCrawlStatus\('new'\)/)
  assert.match(detail, /setCrawlStatus\('updated'\)/)
  assert.match(detail, /setReviewMode\('review'\)/)
})

test('Brand Pulse detail keeps crawl controls out of the compact summary panel', async () => {
  const detail = await readFile(new URL('../app/(app)/brand-mentions/[id]/page.tsx', import.meta.url), 'utf8')
  const css = await readFile(new URL('../app/globals.css', import.meta.url), 'utf8')
  const summaryStart = detail.indexOf('summary={')
  const detailBodyStart = detail.indexOf('>\n          {loadError', summaryStart)
  const summaryBlock = detail.slice(summaryStart, detailBodyStart)
  const crawlSectionIndex = detail.indexOf('<JobSection title="Crawl controls"')
  const dfsInsightsIndex = detail.indexOf('<JobSection title="DFS insights"')

  assert.match(detail, /summary=\{\s*<div className="brand-pulse-summary-controls">/)
  assert.doesNotMatch(summaryBlock, /brand-pulse-crawl-actions/)
  assert.ok(crawlSectionIndex > detailBodyStart)
  assert.ok(crawlSectionIndex < dfsInsightsIndex)
  assert.match(detail, /<JobSection title="Crawl controls" className="brand-pulse-crawl-panel">/)
  assert.match(detail, /brand-pulse-crawl-toolbar/)
  ...
  assert.match(css, /\.brand-pulse-crawl-panel[\s\S]*padding: 12px 14px/)
  assert.match(css, /\.brand-pulse-dfs-selector[\s\S]*background: transparent/)
})
...
test('Brand Pulse detail uses compact coverage and aligned crawl actions', async () => {
  ...
  assert.match(detail, /Run Pulse/)
  assert.doesNotMatch(detail, /Run Crawl/)
  assert.doesNotMatch(detail, /Showing \{visibleMentions\.length\} of \{displayedMentions\.length\} filtered mentions/)
  assert.match(detail, /showDfsInsightDetails/)
  assert.match(detail, /showCrawlHistory/)
  assert.match(detail, /Show crawl history/)
  assert.match(detail, /dfsInsightSnapshotItems/)
  assert.match(detail, /Indexed mentions/)
  assert.doesNotMatch(detail, /brand-pulse-coverage-strip/)
  assert.match(css, /\.brand-pulse-crawl-actions[\s\S]*grid-template-columns: minmax\(0, 1fr\) auto/)
  assert.match(css, /\.brand-pulse-dfs-selector[\s\S]*max-width: none/)
  assert.match(css, /\.brand-pulse-insight-details/)
  assert.doesNotMatch(detail, /brand-pulse-mentions-toolbar/)
  assert.doesNotMatch(detail, /brand-pulse-mentions-actions/)
  assert.doesNotMatch(css, /\.brand-pulse-mentions-toolbar/)
  assert.doesNotMatch(css, /\.brand-pulse-mentions-actions/)
  assert.doesNotMatch(css, /\.brand-pulse-coverage-strip/)
})

test('Brand Pulse detail exposes on-demand DFS summary coverage insight', async () => {
  const detail = await readFile(new URL('../app/(app)/brand-mentions/[id]/page.tsx', import.meta.url), 'utf8')
  const api = await readFile(new URL('../lib/api/brand-mentions.ts', import.meta.url), 'utf8')

  assert.match(api, /getSummaryInsight: \(token: string, id: string\)/)
  assert.match(api, /refreshSummaryInsight: \(token: string, id: string\)/)
  assert.match(api, /getSentimentInsight: \(token: string, id: string\)/)
  assert.match(api, /refreshSentimentInsight: \(token: string, id: string\)/)
  assert.match(detail, /coverageInsight/)
  assert.match(detail, /sentimentInsight/)
  assert.match(detail, /Refresh summary/)
  assert.match(detail, /Refresh sentiment/)
  assert.match(detail, /brandMentionsApi\.refreshSummaryInsight/)
  assert.match(detail, /brandMentionsApi\.refreshSentimentInsight/)
  assert.match(detail, /Indexed citations/)
  assert.match(detail, /Analyzed mentions/)
  assert.match(detail, /Emotion signals/)
  assert.match(detail, /Top countries/)
  assert.match(detail, /Top languages/)
})

test('Brand Pulse exposes review actions suppression rules and profile reports', async () => {
  const api = await readFile(new URL('../lib/api/brand-mentions.ts', import.meta.url), 'utf8')
  const detail = await readFile(new URL('../app/(app)/brand-mentions/[id]/page.tsx', import.meta.url), 'utf8')
  const profile = await readFile(new URL('../app/(app)/brand-mentions/profiles/[id]/page.tsx', import.meta.url), 'utf8')
  const createAlert = await readFile(new URL('../app/(app)/brand-mentions/profiles/[id]/alerts/new/page.tsx', import.meta.url), 'utf8')

  assert.match(api, /getProfileReport/)
  assert.match(api, /updateMentionReview/)
  assert.match(api, /createSuppressionRule/)
  assert.match(api, /listSuppressionRules/)
  assert.match(detail, /handleReviewMention/)
  assert.match(detail, /handleFavoriteMention/)
  assert.match(detail, /handleSuppressMention/)
  assert.match(detail, /Mark wrong mention/)
  assert.match(detail, /wrong mention/)
  assert.match(detail, /Hide domain/)
  assert.match(detail, /Hide group/)
  assert.match(detail, /ExportMenu/)
  assert.match(detail, /exportRowsToGoogleSheets/)
  assert.match(detail, /downloadXlsx/)
  assert.match(detail, /Load more/)
  assert.match(detail, /mentionMatchEvidence/)
  assert.match(detail, /Matched phrase/)
  assert.match(detail, /Visible in/)
  assert.match(detail, /Evidence/)
  assert.match(detail, /brand-pulse-evidence/)
  assert.match(profile, /Export Report/)
  assert.match(profile, /buildReportHtml/)
  assert.match(createAlert, /Scheduled crawl/)
  assert.match(createAlert, /Queue digest/)
  assert.match(createAlert, /crawl_frequency/)
  assert.match(createAlert, /digest_recipients/)
})

test('Supabase client handles missing public auth env without crashing pages', async () => {
  const source = await readFile(new URL('../lib/supabase.ts', import.meta.url), 'utf8')

  assert.match(source, /Authentication service temporarily unavailable/)
  assert.match(source, /missingSupabaseConfig/)
  assert.match(source, /getSession: async \(\) => \(\{ data: \{ session: null \}/)
})
