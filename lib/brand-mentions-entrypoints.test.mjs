...
test('Brand Pulse detail uses compact coverage and aligned crawl actions', async () => {
  ...
  assert.match(css, /\.brand-pulse-mentions-toolbar[\s\S]*position: sticky/)
  assert.match(css, /\.brand-pulse-mentions-actions/)
...
test('Brand Pulse exposes review actions suppression rules and profile reports', async () => {
  ...
  assert.match(detail, /MENTION_PAGE_SIZE_OPTIONS/)
  ...
  assert.match(detail, /brand-pulse-mentions-actions/)
