...
test('CustomSelect exposes one accessible default and compact control contract', () => {
  const source = read('components', 'ui', 'CustomSelect.tsx')
  const globals = read('app', 'globals.css')

  assert.match(source, /export type CustomSelectSize = 'default' \| 'compact'/)
  ...
  assert.match(globals, /\.cs-trigger\s*\{[\s\S]*?height:\s*36px/)
  assert.match(globals, /\.cs-trigger\[data-size="compact"\][\s\S]*?height:\s*32px/)
  assert.match(globals, /\.cs-trigger:focus-visible/)
...
test('FAQ and Intro row type selectors use CustomSelect', () => {
  ...
  assert.match(faq, /<CustomSelect size="compact"[\s\S]*?value=\{row\.page_type\}/)
  ...
  assert.match(intro, /<CustomSelect[\s\S]*?value=\{row\.page_type\}[\s\S]*?size="compact"/)
})
...
test('new-job settings use CustomSelect while preserving special value callbacks', () => {
  ...
  assert.match(meta, /value=\{brandProfileId\} onChange=\{applyBrandProfile\}/)
  assert.match(meta, /function applyBrandProfile\(value: string\) \{[\s\S]*?brandProfiles\.find\(p => p\.id === value\)[\s\S]*?setBrandName\(profile\.data\.brand_name\)/)
  assert.match(meta, /function handleIncludeBrandToggle\(\) \{[\s\S]*?brandProfiles\.find\(p => p\.id === brandProfileId\)[\s\S]*?setBrandName\(profile\.data\.brand_name\)/)
  ...
  assert.match(allInOne, /value=\{brandProfileId\} onChange=\{applyBrandProfile\}/)
  assert.match(allInOne, /function applyBrandProfile\(value: string\) \{[\s\S]*?brandProfiles\.find\(item => item\.id === value\)[\s\S]*?setBrandName\(profileBrandName\)/)
})
...
test('CustomSelect supports grouped options and Group 3 internal selectors use it', () => {
  ...
  assert.match(niche, /import CustomSelect, \{ type CustomSelectSize \} from '\.\/CustomSelect'/)
...
test('GEOPilot forms and result filters use the shared select contract', () => {
  const profileForm = read('components', 'geopilot', 'ProfileForm.tsx')
  const collectionForm = read('app', '(app)', 'geopilot', 'profiles', '[id]', 'collections', 'new', 'page.tsx')
  const profilePage = read('app', '(app)', 'geopilot', 'profiles', '[id]', 'page.tsx')
  const profileStyles = read('components', 'geopilot', 'GeoPilotProfile.module.css')

  for (const source of [profileForm, collectionForm, profilePage]) {
    assert.match(source, /import CustomSelect from '@\/components\/ui\/CustomSelect'/)
    assert.doesNotMatch(source, /<select/)
  }
  assert.match(profilePage, /ariaLabel="Filter by collection method"/)
  assert.match(profilePage, /ariaLabel="Filter by result"/)
  assert.match(profilePage, /size="compact"/)
  assert.doesNotMatch(profileStyles, /\.methodFilter select|\.outcomeFilter select/)
})

test('action dropdowns share the CopyPilot command-menu surface', () => {
  const exportMenu = read('components', 'ui', 'ExportMenu.tsx')
  const intro = read('app', '(app)', 'intro', 'jobs', 'new', 'page.tsx')

  assert.match(exportMenu, /className="cp-menu-item cp-menu-item-rich/)
  assert.match(exportMenu, /className="cp-menu absolute/)
  assert.match(exportMenu, /className="cp-menu-label"/)
  assert.match(intro, /className="cp-menu absolute/)
  assert.match(intro, /className="cp-menu-row group"/)
  assert.match(intro, /className="cp-menu cp-menu-form absolute/)
})
