...
test('schema generator is available in the other navigation section', async () => {
  ...
  const firstOtherEntry = other.match(/\{[^}]*\}/)?.[0] ?? ''

  assert.doesNotMatch(tools, /label:\s*'Schema Generator'/)
  assert.match(firstOtherEntry, /href:\s*'\/schema\/jobs'[\s\S]*label:\s*'Schema Generator'/)
