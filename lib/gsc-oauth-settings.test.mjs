...
test('settings page renders the Indexer boundary note', async () => {
  const source = await settingsSrc()
  assert.match(source, /Keep a service account if you use Indexer/)
  assert.match(source, /Search Console data only/)
})

test('settings page presents Google OAuth as preferred while keeping service account available', async () => {
  const source = await settingsSrc()
  assert.match(source, /Preferred for FAQ, Intro, Meta, and All-in-One/i)
  assert.match(source, /Service account remains available/i)
})

test('settings page explains Google OAuth connection states clearly', async () => {
  const source = await settingsSrc()
  assert.match(source, /gscStatusText/)
  assert.match(source, /Ready for Search Console keyword data/)
...
test('settings page uses a dedicated Search Console icon for the Google connection card', async () => {
  const source = await settingsSrc()
  assert.match(source, /function SearchConsoleIcon/)
  assert.match(source, /SearchConsoleIcon/)
})
