...
test('AIO result page reports the latest deterministic section rerun outcome', async () => {
  const source = await readFile(new URL('../app/(app)/all-in-one/jobs/[id]/page.tsx', import.meta.url), 'utf8')

  assert.match(source, /section_rerun_outcomes/)
  assert.match(source, /Instruction applied/)
  assert.match(source, /Instruction partially applied/)
  assert.match(source, /Instruction blocked/)
  assert.match(source, /rerunOutcome\.message/)
...
test('AIO result page shows additive section plans, guidance versions, and quality diagnostics', async () => {
  ...
  assert.match(source, /adaptive_section_plan\?: AdaptiveSectionPlan\[\]/)
  assert.match(source, /findAdaptiveSectionPlan/)
  assert.match(source, /selectPageCopySectionHeadings/)
  ...
  assert.match(source, /if \(headingSelection\.exportHeading\) lines\.push\(headingSelection\.exportHeading, text, ''\)/)
  assert.match(source, /isVersionedHeadinglessSection/)
  assert.match(source, /expectedHeadingLevel === 'none'/)
  assert.match(source, /else lines\.push\(text, ''\)/)
  ...
  assert.match(source, /const actualHeading = generatedSectionHeading\(text\)/)
  assert.doesNotMatch(source, /const actualHeading = isVersionedPageCopy/)
  assert.match(source, /\{headingSelection\.displayHeading\}/)
  assert.match(source, /Initial heading \(not used\)/)
  assert.match(source, /Initial coverage points \(not used\)/)
