...
const jobPageSrc = () =>
  readFile(new URL('../app/(app)/intro/jobs/[id]/page.tsx', import.meta.url), 'utf8')

test('Intro job result type includes the safe GSC auth method label', async () => {
  const source = await jobPageSrc()
  assert.match(source, /gsc_auth_method\?:/)
})

test('Intro results render a GSC auth badge when the backend provides one', async () => {
  const source = await jobPageSrc()
  assert.match(source, /Google OAuth|Service account/)
  assert.match(source, /row\.gsc_auth_method/)
})

test('Intro job page translates known GSC job errors into helpful UI copy', async () => {
  const source = await jobPageSrc()
