...
test('sidebar wordmark keeps descenders clear of the logo row', async () => {
  ...
  const fontStart = source.indexOf("fontSize: '0.9375rem'")
  const wordmarkStart = source.lastIndexOf('<span style={{', fontStart)
  const wordmarkEnd = source.indexOf('}}>', fontStart)
  assert.notEqual(fontStart, -1)
  assert.notEqual(wordmarkStart, -1)
  assert.notEqual(wordmarkEnd, -1)
  const wordmark = source.slice(wordmarkStart, wordmarkEnd + 3)

  assert.match(wordmark, /display: 'inline-flex'/)
  assert.match(wordmark, /alignItems: 'center'/)
  assert.match(wordmark, /minHeight: 24/)
  assert.match(wordmark, /lineHeight: '24px'/)
  assert.match(wordmark, /paddingBottom: 3/)
})
...
test('sidebar nav labels avoid clipped descenders', async () => {
  ...
  assert.match(source, /className="flex-1 truncate leading-\[1\.2\]"/)
  assert.doesNotMatch(source, /className="flex-1 truncate leading-none"/)
})
