...
test('app shell uses a full-height docked workspace without outer card chrome', async () => {
  ...
  assert.match(css, /\.shell\s*\{[^}]*position:\s*fixed[^}]*inset:\s*0[^}]*overscroll-behavior:\s*none/)
  assert.match(css, /\.workspace\s*\{[^}]*min-height:\s*0[^}]*overflow:\s*hidden/)
  assert.match(css, /\.main\s*\{[^}]*min-height:\s*0[^}]*overflow-y:\s*auto[^}]*overscroll-behavior-y:\s*none/)
...
test('authenticated viewport locking does not alter public-page scrolling', async () => {
  const appLayout = await readFile(
    new URL('../components/layout/AppLayout.tsx', import.meta.url),
    'utf8',
  )
  const appShellCss = await readFile(
    new URL('../components/layout/AppLayout.module.css', import.meta.url),
    'utf8',
  )
  ...
  assert.match(appLayout, /className=\{styles\.shell\}/)
  assert.doesNotMatch(publicLayout, /AppLayout|styles\.shell/)
  assert.doesNotMatch(appShellCss, /:global\((?:html|body)\)/)
})
