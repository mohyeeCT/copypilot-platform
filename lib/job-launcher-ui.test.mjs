...
const visibleNewJobPages = [
  ...
  ['schema', 'app', '(app)', 'schema', 'jobs', 'new', 'page.tsx'],
  ['indexer', 'app', '(app)', 'indexer', 'jobs', 'new', 'page.tsx'],
]

const aiNewJobPages = visibleNewJobPages.filter(([name]) => name !== 'indexer')
...
test('shared job launcher components exist with shell, sections, and summary', () => {
  ...
  assert.match(shell, /function JobSummaryPills/)
...
test('visible batch job pages keep primary run actions in the launcher header', () => {
  for (const [name, ...parts] of visibleNewJobPages) {
    ...
    assert.match(source, /Run Job/, `${name} should use the standard run action label`)
    assert.doesNotMatch(source, /Generate Meta Copy|Run All in One/, `${name} should not use tool-specific run action labels`)
  }
})

test('visible batch job pages present hero AI and context as summary pills', () => {
  for (const [name, ...parts] of aiNewJobPages) {
    const source = read(...parts)
    assert.match(source, /JobSummaryPills/, `${name} should use visual summary pills`)
    assert.match(source, /cleanProviderLabel/, `${name} should hide provider parenthetical labels in the hero`)
    assert.match(source, /cleanModelLabel/, `${name} should hide raw model IDs in the hero`)
  }
})

test('indexer new-job page uses summary pills without AI provider labels', () => {
  const source = read('app', '(app)', 'indexer', 'jobs', 'new', 'page.tsx')
  assert.match(source, /JobSummaryPills/)
  assert.match(source, /Indexing API/)
  assert.doesNotMatch(source, /cleanProviderLabel/)
  assert.doesNotMatch(source, /cleanModelLabel/)
})
