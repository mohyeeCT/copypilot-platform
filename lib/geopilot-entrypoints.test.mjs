...
test('GEOPilot exports trends methods citations costs and prompt history', async () => {
  ...
  assert.match(profile, /All data \(\.zip\)[\s\S]*Prompt-level history[\s\S]*Daily trends[\s\S]*Method comparisons/)
  assert.match(profile, /Citation history[\s\S]*Verified citation gaps[\s\S]*Provider costs/)
