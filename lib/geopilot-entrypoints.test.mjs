...
test('GEOPilot exports trends methods citations costs and prompt history', async () => {
  const profile = await readFile(new URL('../app/(app)/geopilot/profiles/[id]/page.tsx', import.meta.url), 'utf8')
  const api = await readFile(new URL('./api/geopilot.ts', import.meta.url), 'utf8')
  const exports = await readFile(new URL('./geopilot-exports.ts', import.meta.url), 'utf8')
  const exportMenu = await readFile(new URL('../components/ui/ExportMenu.tsx', import.meta.url), 'utf8')

  assert.match(profile, /All data \(\.zip\)/)
  assert.match(exports, /Prompt-level history[\s\S]*Daily trends[\s\S]*Method comparisons/)
  assert.match(exports, /Citation history[\s\S]*Verified citation gaps[\s\S]*Provider costs/)
  assert.match(profile, /downloadGeoPilotExport/)
  assert.match(profile, /ExportMenu[\s\S]*Excel workbook[\s\S]*Google Sheets/)
  assert.match(profile, /downloadGeoPilotWorkbook/)
  assert.match(profile, /exportRowsToGoogleSheets/)
  assert.doesNotMatch(profile, /ariaLabel="Export GEOPilot data"/)
  assert.match(api, /expandedExportUrl:[\s\S]*export\.zip[\s\S]*exports\/\$\{dataset\}\.csv/)
  assert.match(api, /exportBundleUrl:[\s\S]*export-bundle\.json/)
  assert.match(api, /fetchGeoPilotExportBundle/)
  assert.match(profile, /fetchGeoPilotExportBundle\(accessToken, id, days\)/)
  assert.match(exportMenu, /downloadActions\?: ExportMenuAction\[\]/)
})

test('GEOPilot exposes citation intelligence provider health and a verified AIO handoff', async () => {
  const profile = await readFile(new URL('../app/(app)/geopilot/profiles/[id]/page.tsx', import.meta.url), 'utf8')
  const api = await readFile(new URL('./api/geopilot.ts', import.meta.url), 'utf8')
  const aio = await readFile(new URL('../app/(app)/all-in-one/jobs/new/page.tsx', import.meta.url), 'utf8')

  assert.match(profile, /Citation intelligence[\s\S]*Owned[\s\S]*Competitor[\s\S]*Third-party/)
  assert.match(profile, /Provider health[\s\S]*parser, model-catalog, and citation-change monitoring/i)
  assert.match(profile, /Verified citation gaps[\s\S]*Create AIO recommendation/)
  assert.match(profile, /geopilotApi\.createAioRecommendation\(accessToken, id, gap\.run_id\)/)
  assert.match(api, /citation-intelligence/)
  assert.match(api, /provider-alerts/)
  assert.match(api, /aio-recommendations/)
  assert.match(aio, /geopilot_recommendation/)
  assert.match(aio, /setClientBrief\(String\(draft\.client_brief/)
  assert.match(aio, /Confirm the destination URL and keyword before running it/)
  assert.doesNotMatch(aio, /setRows\([^\n]*keyword:\s*draft/)
})

test('GEOPilot client reports keep tokens out of request paths and honor section scope', async () => {
  const dialog = await readFile(new URL('../components/geopilot/ReportShareDialog.tsx', import.meta.url), 'utf8')
  const access = await readFile(new URL('../components/geopilot/public/ReportAccess.tsx', import.meta.url), 'utf8')
  const report = await readFile(new URL('../components/geopilot/public/PublicGeoPilotReport.tsx', import.meta.url), 'utf8')
  const monitor = await readFile(new URL('../components/geopilot/SourceMonitorPanel.tsx', import.meta.url), 'utf8')
  const exchange = await readFile(new URL('../app/api/public/geopilot/report-exchange/route.ts', import.meta.url), 'utf8')

  assert.match(dialog, /\/reports\/geopilot\/open#\$\{encodeURIComponent\(data\.token\)\}/)
  assert.match(dialog, /hasVisibleSection/)
  assert.match(access, /window\.location\.hash/)
  assert.match(access, /window\.history\.replaceState/)
  assert.match(report, /'trends' in report[\s\S]*'costs' in report/)
  assert.match(report, /tab === 'Trends'[\s\S]*tab === 'Costs'/)
  assert.match(monitor, /const href = safeUrl\(item\.url\)[\s\S]*href \? <a/)
  assert.match(exchange, /payload\.detail === 'passcode_required'/)
  assert.doesNotMatch(exchange, /detail:\s*payload\.detail\s*\|\|/)
})
