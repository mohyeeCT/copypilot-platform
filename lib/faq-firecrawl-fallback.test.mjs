...
test('FAQ new jobs keep Firecrawl fallback off and submit only a safe boolean', async () => {
  ...
  assert.match(source, /value: 'jina', label: 'Jina'/)
  assert.match(source, /value: 'firecrawl', label: 'Firecrawl'/)
  assert.match(source, /has_firecrawl_key/)
  assert.match(source, /scrape_provider: scrapeProvider/)
  assert.match(source, /firecrawl_fallback: scrapePages && scrapeProvider === 'jina' && firecrawlFallback && firecrawlKeyConfigured/)
  assert.match(source, /scrapePages && scrapeProvider === 'jina'/)
  assert.match(source, /Add a Firecrawl API key in Settings before using Firecrawl as the primary scraper/)
...
test('FAQ results offer Firecrawl rerun only for failed page scraping', async () => {
  ...
  assert.match(source, /jobStartedWithFirecrawl = job\?\.settings\?\.scrape_provider === 'firecrawl'/)
  assert.match(source, /selectedScrapeFailed && !jobStartedWithFirecrawl && firecrawlKeyConfigured/)
