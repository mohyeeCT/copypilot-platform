...
test('All-in-One meta length display uses relaxed Meta limits', async () => {
  const source = await jobPageSrc()
  assert.match(source, /row\.title_length > 90/)
  assert.match(source, /row\.description_length > 200/)
  assert.match(source, /row\.title_length\}\/90/)
  assert.match(source, /row\.description_length\}\/200/)
  assert.doesNotMatch(source, /row\.title_length > 60/)
  assert.doesNotMatch(source, /row\.description_length > 155/)
})
