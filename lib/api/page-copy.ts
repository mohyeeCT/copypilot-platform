...
const BASE = (
  process.env.NEXT_PUBLIC_PAGE_COPY_API_URL || 'https://page-copy-saas-backend-production.up.railway.app'
).replace(/\/+$/, '')

const f = (path: string, token: string, opts?: RequestInit) => apiFetch(BASE, path, token, opts)

export const pageCopyApi = {
  runJob:       (token: string, payload: object) => f('/api/page-copy/run', token, { method: 'POST', body: JSON.stringify(payload) }),
  ...
  getTemplates: (token: string)                  => f('/api/page-copy/templates', token),
