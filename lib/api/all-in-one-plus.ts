...
const BASE = (
  process.env.NEXT_PUBLIC_AIO_PLUS_API_URL || 'https://all-in-one-plus-saas-backend-production.up.railway.app'
).replace(/\/+$/, '')

const f = (path: string, token: string, opts?: RequestInit) => apiFetch(BASE, path, token, opts)

export interface AioPlusPageCopyGuidanceProfile {
  id: string
  label: string
  description: string
  version: string
...
export interface AioPlusPageCopyCapabilities {
  enabled: boolean
  profiles: AioPlusPageCopyGuidanceProfile[]
  default_profile_id: string
  policy_versions?: Record<string, string>
}

export const aioPlusApi = {
  ...
  getPageCopyCapabilities: (token: string)       =>
    f('/api/all-in-one/page-copy-capabilities', token, { cache: 'no-store' }) as Promise<AioPlusPageCopyCapabilities>,
