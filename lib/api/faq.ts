...
export const faqApi = {
  runJob:    (token: string, payload: object) => f('/api/faq/run', token, { method: 'POST', body: JSON.stringify(payload) }),
  ...
  rerunRow: (token: string, id: string, index: number, kwOverride?: string, scraperOverride?: 'firecrawl') =>
    f(`/api/jobs/${id}/rerun-row/${index}`, token, {
      method: 'POST',
      body: JSON.stringify({
        keyword_override: kwOverride || '',
        scraper_override: scraperOverride || '',
      }),
    }),
