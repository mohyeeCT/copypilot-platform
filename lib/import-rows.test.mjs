...
test('reports the physical source row after a quoted multiline value', () => {
  const text = [
    'url,keyword,page_type,h1',
    '"https://example.com/a",keyword,blog,"First line',
    'second line"',
    'example.com/no-protocol,invalid,service,Heading',
  ].join('\n')
  ...
  assert.equal(result.rejectedRows[0].rowNumber, 4)
})
...
test('can opt into mapping URL, page template, H1 rows when keyword is omitted', () => {
  const result = parseImportedRows(
    'https://example.com/a\tservice_lp\tService H1',
    createCopyRowImportSchema(
      { page_type: 'category' },
      false,
      {
        pageTypeValues: ['service_lp', 'category', 'product', 'location', 'blog', 'brand'],
        positionalLayouts: [
          {
  ...
  assert.deepEqual(result.rows, [
    { url: 'https://example.com/a', keyword: '', page_type: 'service_lp', h1: 'Service H1' },
  ])
  assert.deepEqual(result.rejectedRows, [])
  assert.deepEqual(result.notices, [{
    rowNumber: 1,
    message: 'Keyword column was omitted; mapped the second column as page template.',
  }])
})

test('can opt into mapping URL, keyword, H1 rows when page template is omitted', () => {
  const result = parseImportedRows(
    'https://example.com/a\tseo audit services\tService H1',
    createCopyRowImportSchema(
      { page_type: 'service_lp' },
      false,
      {
        pageTypeValues: ['service_lp', 'category', 'product', 'location', 'blog', 'brand'],
        positionalLayouts: [
  ...
  assert.deepEqual(result.rows, [
    { url: 'https://example.com/a', keyword: 'seo audit services', page_type: 'service_lp', h1: 'Service H1' },
  ])
  assert.deepEqual(result.rejectedRows, [])
  assert.deepEqual(result.notices, [{
    rowNumber: 1,
    message: 'Page template column was omitted; used the selected default template.',
  }])
})

test('recognizes page template header aliases', () => {
  const result = parseImportedRows(
    'url,page template,h1\nhttps://example.com/a,blog,Article H1',
    createCopyRowImportSchema(
      { page_type: 'service_lp' },
      false,
      { pageTypeValues: ['service_lp', 'category', 'product', 'location', 'blog', 'brand'] },
  ...
  assert.deepEqual(result.rows, [
    { url: 'https://example.com/a', keyword: '', page_type: 'blog', h1: 'Article H1' },
  ])
  assert.deepEqual(result.rejectedRows, [])
})

test('normalizes accepted page type casing for pasted rows', () => {
  const text = 'https://www.perdue.com/the-table-blog/entertaining/occasions/3-crowd-pleasing-apps-that-ll-leave-them-wanting-more\tchicken party appetizers\tBlog\t3 Crowd-Pleasing Apps That’ll Leave Them Wanting More'
  const result = parseImportedRows(
    text,
    createCopyRowImportSchema(
      { page_type: 'general' },
      false,
      {
        pageTypeValues: ['general', 'category', 'product', 'service', 'location', 'blog', 'brand'],
        positionalLayouts: [
          {
            keys: ['url', 'page_type', 'h1'],
            match: [{ index: 1, columnKey: 'page_type' }],
  ...
  assert.deepEqual(result.rejectedRows, [])
  assert.equal(result.rows[0].page_type, 'blog')
})
