...
export default function DashboardPage() {
  ...
  const load = useCallback(async () => {
    ...
    if (!session) return
    try {
      const data = await pageCopyApi.listJobs(session.access_token)
      setJobs(data)
    } catch {}
    setLoading(false)
  }, [])
  ...
  async function handleRename(id: string, name: string) {
    ...
    if (!session) return
    try {
      await pageCopyApi.renameJob(session.access_token, id, name.trim())
  ...
  async function handleDelete(id: string) {
    ...
    const { data: { session } } = await sb.auth.getSession()
    if (!session) return
    await pageCopyApi.deleteJob(session.access_token, id)
  ...
  return (
    <AppLayout title="Page Copy">
      <div className="max-w-5xl">
        <div className="flex items-center justify-between mb-8">
          <div>
            <h1 className="text-2xl font-bold">Jobs</h1>
            <p className="text-muted text-sm mt-1">Page copy generation jobs</p>
          </div>
          <div className="flex gap-2">
            <button onClick={load} className="btn-ghost flex items-center gap-2">
              <RefreshCw size={14} /> Refresh
            </button>
            <button onClick={() => router.push('/page-copy/jobs/new')} className="btn-primary flex items-center gap-2">
        ...
        {loading ? (
          ...
          <div className="card p-16 text-center">
            ...
            <button onClick={() => router.push('/page-copy/jobs/new')} className="btn-primary">
              Run your first job
            </button>
              ...
              <tbody>
                {jobs.map((job, i) => (
                  <tr
                    ...
                    onClick={() => router.push(`/page-copy/jobs/${job.id}`)}
                  >
                      ...
                      <div className="flex items-center gap-1 justify-end" onClick={e => e.stopPropagation()}>
                        ...
                        <button
                          onClick={async () => {
                            ...
                            const { data: { session } } = await sb.auth.getSession()
                            if (!session) return
                            const res = await pageCopyApi.duplicateJob(session.access_token, job.id)
