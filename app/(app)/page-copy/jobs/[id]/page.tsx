    ...
    <AppLayout title="Page Copy">
      <div className="max-w-5xl mx-auto">
        ...
        <div className="flex items-center gap-3 mb-6">
          <Link href="/page-copy/jobs" className="text-muted hover:text-text transition-colors">
            <ArrowLeft size={18} />
          </Link>
              ...
              <button onClick={async () => {
                ...
                try {
                  ...
                  if (session) {
                    ...
                    await pageCopyApi.rerunRows(session.access_token, job.id, indices)
                    const refreshed = await pageCopyApi.getJob(session.access_token, job.id)
                  ...
                  <div className="flex items-center gap-2 shrink-0">
                    ...
                    {rerunning === i ? (
                      ...
                      <button onClick={async e => {
                        ...
                        const { data: { session } } = await sb.auth.getSession()
                        if (session) {
                          await pageCopyApi.rerunRow(session.access_token, job.id, i)
                          const poll = setInterval(async () => {
                            const updated = await pageCopyApi.getJob(session.access_token, job.id)
                  ...
                  <div className="px-4 pb-4 space-y-4 bg-bg/40">
                    ...
                    {row.section_results && Object.keys(row.section_results).length > 0 && (
                      <div className="space-y-3">
                        <p className="text-xs text-muted uppercase tracking-wider">Sections</p>
