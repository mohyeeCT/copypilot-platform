...
export default function DashboardPage() {
  ...
  const load = useCallback(async () => {
    ...
    if (!session) return
    try {
      const data = await faqApi.listJobs(session.access_token)
      setJobs(data)
    } catch {}
    setLoading(false)
  }, [])
  ...
  async function handleRename(id: string, name: string) {
    ...
    if (!session) return
    try {
      await faqApi.renameJob(session.access_token, id, name.trim())
  ...
  async function handleDelete(id: string) {
    ...
    const { data: { session } } = await sb.auth.getSession()
    if (!session) return
    await faqApi.deleteJob(session.access_token, id)
      ...
      <div className="max-w-5xl">
        <div className="flex items-center justify-between mb-8">
          <div>
            ...
            <p className="text-muted text-sm mt-1">FAQ generation jobs</p>
          </div>
          <div className="flex gap-2">
            <button onClick={load} className="btn-ghost flex items-center gap-2">
              <RefreshCw size={14} /> Refresh
            </button>
            <button onClick={() => router.push('/faq/jobs/new')} className="btn-primary flex items-center gap-2">
        ...
        {loading ? (
          ...
          <div className="card p-16 text-center">
            ...
            <button onClick={() => router.push('/faq/jobs/new')} className="btn-primary">
              Run your first job
            </button>
              ...
              <tbody>
                {jobs.map((job, i) => (
                  <tr
                    ...
                    onClick={() => router.push(`/faq/jobs/${job.id}`)}
                  >
                      ...
                      <div className="flex items-center gap-1 justify-end" onClick={e => e.stopPropagation()}>
                        ...
                        <button
                          onClick={async () => {
                            ...
                            const { data: { session } } = await sb.auth.getSession()
                            if (!session) return
                            const res = await faqApi.duplicateJob(session.access_token, job.id)
                            if (res?.job_id) router.push(`/faq/jobs/${res.job_id}`)
                          }}
                          className="p-1.5 text-muted hover:text-accent transition-colors"
                          title="Duplicate job"
                        >
