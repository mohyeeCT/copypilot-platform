...
export default function DashboardPage() {
  ...
  const load = useCallback(async () => {
    ...
    if (!session) return
    try {
      const data = await metaApi.listJobs(session.access_token)
      setJobs(data)
    } catch {}
    setLoading(false)
  }, [])
  ...
  async function handleRename(id: string, name: string) {
    ...
    if (!session) return
    try {
      await metaApi.renameJob(session.access_token, id, name.trim())
  ...
  async function handleDelete(id: string) {
    ...
    const { data: { session } } = await sb.auth.getSession()
    if (!session) return
    await metaApi.deleteJob(session.access_token, id)
      ...
      <div className="max-w-5xl">
        <div className="flex items-center justify-between mb-8">
          <div>
            ...
            <p className="text-muted text-sm mt-1">Meta copy generation jobs</p>
          </div>
          <div className="flex gap-2">
            <button onClick={load} className="btn-ghost flex items-center gap-2">
              <RefreshCw size={14} /> Refresh
            </button>
            <button onClick={() => router.push('/meta/jobs/new')} className="btn-primary flex items-center gap-2">
        ...
        {loading ? (
          ...
          <div className="card p-16 text-center">
            ...
            <button onClick={() => router.push('/meta/jobs/new')} className="btn-primary">
              Run your first job
            </button>
              ...
              <tbody>
                {jobs.map((job, i) => (
                  <tr
                    ...
                    onClick={() => router.push(`/meta/jobs/${job.id}`)}
                  >
                      ...
                      <div className="flex items-center gap-1 justify-end" onClick={e => e.stopPropagation()}>
                        ...
                        <button
                          onClick={async () => {
                            ...
                            const { data: { session } } = await sb.auth.getSession()
                            if (!session) return
                            const res = await metaApi.duplicateJob(session.access_token, job.id)
                            if (res?.job_id) router.push(`/meta/jobs/${res.job_id}`)
                          }}
                          className="p-1.5 text-muted hover:text-accent transition-colors"
                          title="Duplicate job"
                        >
