...
import { applyAioOutputSelection, type AioOutputField } from '@/lib/all-in-one-plus-output-selection'
...
import { aioPlusApi, type AioPlusPageCopyCapabilities } from '@/lib/api/all-in-one-plus'
...
export default function NewAIOJob() {
  ...
  const [pageCopyCapabilities, setPageCopyCapabilities] = useState<AioPlusPageCopyCapabilities | null>(null)
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      const [s, metadata, bp, tmpl, pageCopyCapabilityResult, recommendationResult] = await Promise.all([
        ...
        aioPlusApi.getTemplates(session.access_token),
        aioPlusApi.getPageCopyCapabilities(session.access_token).catch(() => null),
  ...
  async function handleRun() {
    ...
    const payload = {
      name: jobName.trim() || `AIO+ — ${validRows.length} URLs`,
    ...
    try {
      const data = await aioPlusApi.runJob(session.access_token, payload)
      router.push(`/all-in-one-plus/jobs/${data.job_id}`)
    } catch (e: unknown) {
  ...
  return (
    <AppLayout title="AIO+">
      <div className={`max-w-full ${styles.newPage}`}>
        <Link href="/all-in-one-plus/jobs" className={styles.backLink}>
          <ArrowLeft size={16} /> Back to AIO+ jobs
        </Link>
        {handoffNotice ? (
          ...
          <div className={`space-y-4 ${styles.composerFooter}`}>
            ...
            <div className="card p-4 bg-warning/5 border-warning/20">
              <p className="text-xs text-muted">
                <span className="text-warning font-medium">Note:</span> AIO+ jobs are the longest-running jobs in the platform. A single URL with all three outputs enabled can take 5-15 minutes. For a 10-URL job, budget 1-2 hours. Run overnight for large batches.
              </p>
            </div>
          </div>
        </JobLauncherShell>
