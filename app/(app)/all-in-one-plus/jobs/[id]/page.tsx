...
import aioStyles from '@/components/all-in-one-plus/AllInOnePlusWorkspace.module.css'
...
export default function AllInOneJobPage() {
  ...
  const load = useCallback(async () => {
    ...
    try {
      setJob(await aioPlusApi.getJob(session.access_token, id as string))
    } catch (error) {
  ...
  async function handleCancel() {
    ...
    try {
      ...
      const { data: { session } } = await sb.auth.getSession()
      if (session) {
        await aioPlusApi.cancelJob(session.access_token, job.id)
        await load()
      }
    } catch (error) {
  ...
  function buildGoogleDocBody() {
    const lines: string[] = [job?.name || 'AIO+ Results', `Rows: ${job?.completed_rows ?? 0}/${job?.total_rows ?? 0}`, '']
    results.forEach((row, index) => {
  ...
  async function exportGoogleDocs() {
    ...
    try {
      await exportRowsToGoogleDocs({
        title: `${job.name || 'AIO+ results'} - AIO+`,
  ...
  async function startRowRerun(index: number, scraperOverride?: 'firecrawl') {
    ...
    try {
      ...
      await aioPlusApi.rerunRow(session.access_token, job.id, index, scraperOverride)
      const poll = window.setInterval(async () => {
        try {
          const updated = await aioPlusApi.getJob(session.access_token, job.id)
  ...
  async function rerunSelectedRows() {
    ...
    try {
      ...
      await aioPlusApi.rerunRows(session.access_token, job.id, indices)
      rerunStarted = true
      const refreshed = await aioPlusApi.getJob(session.access_token, job.id)
  ...
  async function rerunSection(rowIndex: number, name: string) {
    ...
    try {
      ...
      const { data: { session } } = await sb.auth.getSession()
      if (!session) throw new Error('Session unavailable')
      await aioPlusApi.rerunSection(session.access_token, job.id, rowIndex, name, reviewerInstruction[sectionKey] || '')
      const poll = window.setInterval(async () => {
        try {
          const updated = await aioPlusApi.getJob(session.access_token, job.id)
      ...
      <div className={styles.jobPage}>
        <header className={styles.pageHeader}>
          <div className={styles.pageHeaderCopy}>
            <Link href="/all-in-one-plus/jobs" className={styles.backButton}><ArrowLeft size={14} /> All AIO+ jobs</Link>
            <span className={styles.eyebrow}>AIO+ job</span>
        ...
        {results.length > 0 && (
          <>
            <section className={styles.metricStrip} aria-label="AIO+ result summary">
            ...
            <div className={styles.toolbar}>
              <div className={styles.toolbarGroup}>
                <label className="flex items-center gap-2 text-xs text-muted">
                  <StyledCheckbox
                    ariaLabel="Select all AIO+ result rows"
            ...
            <div className={styles.reviewWorkspace}>
              <section className={styles.resultQueue}>
                ...
                <div className={styles.queueTools}>
                  <label className={styles.searchField}>
                    <Search size={14} />
                    <span className={styles.srOnly}>Search AIO+ results</span>
                  ...
                  <div className={styles.resultFilters} role="tablist" aria-label="Filter AIO+ results">
                    ...
                    return (
                      <article key={`${row.url}-${index}`} className={`${styles.resultRow} ${selectedIndex === index ? styles.resultRowActive : ''} ${newlyUpdated.has(index) ? 'row-flash' : ''}`}>
                        <div className={styles.resultCheckbox}>
                          <StyledCheckbox
                            ariaLabel={`Select AIO+ result row ${index + 1}`}
              ...
              {selectedResult && (
                <section className={styles.resultDetail}>
                  ...
                  <nav className={styles.detailTabs} aria-label="AIO+ result detail">
                    {([
                      ['overview', 'Overview'],
                      ['strategy', 'Strategy'],
                      ['meta', 'Meta'],
                  ...
                  {detailTab === 'quality' && (
                    <div className={styles.detailBody}>
                      ...
                      <section className={styles.lengthChecks} aria-label="AIO+ quality facts">
                      ...
                      {(selectedResult.page_copy_guidance?.id || selectedResult.page_quality_policy_version || selectedResult.adaptive_policy_version || selectedResult.owned_page_mapping_version) && (
                        <>
                          ...
                          <section className={styles.lengthChecks} aria-label="AIO+ generation policy">
                            <div>
